...
import {
  ...
  updateSubscriptionsID,
} from "../store/user.slice";
import { StoreApp, useAppDispatch } from "../store";
import {
  fetchChangeResponded,
  fetchChangeVacancyIsActive,
} from "../store/vacancies.slice";
...
export const VacancyCard: FC<VacancyCardProps> = ({ ...props }) => {
  ...
  const check =
    props.title === "Cancel" || !subscribeVacanciesID.includes(props.data.id);
  ...
  const onClickChangeIsActive = () => {
    if (props.title === "Cancel") {
      dispatch(
        fetchChangeVacancyIsActive({ id: props.data.id, active: false }),
      );
    } else {
        ...
        <AnimatePresence>
          {role === "applicant" ? (
            <ModalVacancy
              ...
              onClick={
                props.title === "Cancel"
                  ? onClickCancelVacancie
                  : onClickSubscribeVacancie
              }
              title={props.title}
            />
          ) : (
